...
function tfidfSearch(query: string, docs: TfIdfDocument[], limit: number = 20): SearchResult[] {
...
server.tool(
  ...
  "Search agent memory using TF-IDF ranking. Returns a compact index of matching sections with relevance scores. Use memory_detail to fetch full content of interesting results. Searches MEMORY.md + memory/*.md files.",
  {
    ...
    enhanced: z.boolean().optional().default(false).describe(
      "If true, includes full snippets for top results (uses more tokens). Default: compact index only."
    ),
  },
  { title: "Swarm Memory Search (TF-IDF)", readOnlyHint: true },
  async ({ query, memory_dir, limit, enhanced }) => {
    try {
      ...
      const rows = results.map((r, i) =>
        `| ${i + 1} | ${r.file}:${r.line} | ${r.section.slice(0, 40)} | ${r.score.toFixed(2)} |`
      );
...
server.tool(
  ...
  { title: "Memory Detail", readOnlyHint: true },
  async ({ file, line, lines: lineCount, memory_dir }) => {
    try {
      ...
      if (file === "MEMORY.md") {
        ...
        if (existsSync(parentPath)) filePath = parentPath;
        else if (existsSync(cwdPath)) filePath = cwdPath;
        else return { content: [{ type: "text" as const, text: `❌ MEMORY.md not found.` }] };
      } else {
      ...
      return {
        content: [
          { type: "text" as const, text: `Error reading memory: ${(error as Error).message}` },
        ],
      };
    }
  }
...
server.tool(
  ...
  {
    action: z.enum(["init", "push", "pull", "status"]).describe(
      "init: initialize memory repo & connect remote; push: commit & push local changes; pull: fetch & merge remote changes; status: show sync status"
    ),
    repo_url: z
      .string()
      .optional()
  ...
  { title: "Swarm Memory Sync", readOnlyHint: false },
  async ({ action, repo_url, memory_dir, agent_name, message }) => {
    ...
    function git(cmd: string, cwd?: string): string {
      try {
        ...
        throw new Error(`git ${cmd} failed: ${(e as Error).message}`);
      }
    }
    ...
    try {
      ...
      if (action === "status") {
        ...
        let remote = "none";
        try { remote = git("remote get-url origin", dir); } catch { /* no remote */ }
      ...
      return {
        content: [{
          type: "text" as const,
          text: `❌ Swarm Memory sync error: ${(error as Error).message}`,
        }],
      };
    }
  }
